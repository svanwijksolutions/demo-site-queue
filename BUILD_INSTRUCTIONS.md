...
Implementatie meertaligheid: vertaalstrings in `i18n/nl.json`, `i18n/en.json` (etc.), elk vertaalbaar element krijgt `data-i18n="sleutel"`. JS leest voorkeurstaal (localStorage, anders browsertaal, anders NL) en wisselt tekst clientside, GEEN aparte HTML-pagina's per taal, GEEN reload. Taalswitcher (staande regel sinds 03-08-2026, zie RULES.md sectie "Taal"): op desktop (~768px en breder) ALTIJD een uitklapbare dropdown met een vlagicoon per taal, geen pill/segmented-control meer. Op mobiel mag een eenvoudigere, compacte keuze blijven. Voeg `hreflang`-alternates toe. Vertaal ALLE zichtbare tekst mee, geen half vertaalde pagina's.
